...
public class CallableStatementDemo {
    public static void main(String[] args) throws Exception {
        ...
        cs.setString(1,"Ravi");
        cs.setDate(2,new Date(1963,3,13));
        cs.setDate(3,new Date(1993,7,13));         cs.setString(4,"M");
        cs.setString(5,"C");         cs.setString(6,"Assembly");         cs.setFloat(7,4100);         cs.execute();
        System.out.println("Successfully Inserted");
        Statement st =
                con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
        ResultSet rs = st.executeQuery("select * from programmer where pname='Ravi';");
        int cnt=rs.getMetaData().getColumnCount();         int i=1;
        System.out.println("Inserted Details are:");         while(rs.next())
        {
            while(i<=cnt){
